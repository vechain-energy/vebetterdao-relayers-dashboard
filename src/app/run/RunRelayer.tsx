...
import { getNodePoolForEnv } from "@/config/nodeUrls";
...
export function RunRelayer() {
  ...
  const handleStart = useCallback(async () => {
    ...
    const nodePool = getNodePoolForEnv(
      network === "mainnet" ? "mainnet" : "testnet-staging",
    );
    let nodeIndex = 0;
    config.nodeUrl = nodePool[0] ?? config.nodeUrl;
